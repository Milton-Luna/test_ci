...
export class ReviewsSummaryController {
  ...
  @Roles('ADMIN')
  @ApiOperation({ summary: 'Generar reporte semanal IA para un negocio (admin)' })
  ...
  @UseGuards(JwtAuthGuard, RolesGuard)
  @Roles('ADMIN','owner')
  @ApiOperation({ summary: 'Regenerar reseña general IA de todas las reviews (admin)' })
  @ApiResponse({ status: 200, description: 'Reseña general generada y guardada en business.ai_reviews_summary' })
  regenerateGeneralSummary(@Param('businessId', ParseIntPipe) businessId: number) {
    return this.summaryService.generateGeneralSummary(businessId);
  }


  @Get('business/:businessId/general')
  @ApiOperation({ summary: 'Obtener la reseña general IA guardada de un negocio (público)' })
  @ApiResponse({ status: 200, description: 'Reseña general guardada' })
  getSavedGeneralSummary(@Param('businessId', ParseIntPipe) businessId: number) {
