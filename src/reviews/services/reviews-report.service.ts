...
export class ReviewsReportService {
  ...
  async reportReview(reviewId: number, user: any, dto: CreateReviewReportDto) {
    ...
    try {
      ...
      if (
        review.report_count >= this.AUTO_HIDE_THRESHOLD &&
        !review.is_hidden_by_moderation
      ) {
      ...
      if (wasAutoHidden) {
        await this.reviewsService.updateBusinessRating(
  ...
  async moderateReview(reviewId: number, dto: ResolveReportDto) {
    ...
    if (!review)
      throw new NotFoundException('La reseña no existe o ya fue eliminada.');
    ...
    if (dto.action === ModerationAction.DELETE) {
      ...
      try {
        ...
        console.error(
          `No se pudo enviar correo de penalización a ${userEmail}:`,
          error,
        );
      }
